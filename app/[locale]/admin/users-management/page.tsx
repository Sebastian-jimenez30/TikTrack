...
export default async function Index({
  ...
  const pageData = (await axios.get(ROUTES_API.USER_MANAGEMENT_INDEX + `?${query}`, {
  ...
  const emptyRows = Array.from({ length: 8 - users.length })
          ...
          {users.map((user:any) => {
            return (
              <tr key={user.id} className="bg-white border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className={`px-6 py-4 ${user.status === "active" ? "text-green-500" : "text-red-500"}`}>
                    {user.status === "active" ? t("active") : t("inactive")}
                  </td>
                  <td className="flex items-center px-6 py-4">
                    <Button variant="primary" href={ROUTES.MANAGEMENT_DETAIL + `?userId=${user.id}` as Pathname}>
                      {t("manage")}
                    </Button>
                  </td>
              </tr>
